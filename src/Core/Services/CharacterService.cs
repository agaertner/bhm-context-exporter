...
namespace Nekres.Stream_Out.Core.Services {
    internal class CharacterService : ExportService
    {
        ...
        public CharacterService(SettingCollection settings) : base(settings) 
        {
            ...
            _deathsAtResetWeekly = settings.DefineSetting($"{this.GetType().Name}_deaths_weekly_reset", -1);
            _deathsAtResetDaily = settings.DefineSetting($"{this.GetType().Name}_deaths_daily_reset", -1);
        }
